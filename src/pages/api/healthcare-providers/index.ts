...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!session) {
    if (req.method === 'GET') {
      return getHealthcareProvidersPublic();
    }
  ...
  async function getHealthcareProvidersPublic() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'healthcare_provider');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.healthcare_provider.count(countOptions as unknown),
      prisma.healthcare_provider.findMany({
        take: limit,
        skip: offset,
        ...(order?.length && {
